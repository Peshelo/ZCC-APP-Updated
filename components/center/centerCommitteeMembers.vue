...
// import tabheriAddMember from './tabheriAddMember.vue'
  ...
  methods:{
    ...
    async handleSubmit(){
      ...
      // Your code for handling the form submission
          ...
          const URL = `http://13.244.64.153:7635/centers/create/committeeMembers?userId=${this.currentMember.id}&position=${this.currentPosition}&centerId=${this.id}`;
      try{
  ...
  console.log("Error:",err.message)
      ...
      // Your code for handling the form submission
          ...
          const URL = `http://13.244.64.153:7635/admin/create?UserId=${memberId}&scopeId=${scope}&scope=CENTER`;
      try{
       await axios.post(URL,
       {
        headers: 
  ...
  console.log("Error:",err.message)
   ...
   downloadCSV(csv, filename) {
    ...
    async getMembers() {
        ...
        this.loading = true;
        const URL= `http://13.244.64.153:7635/centers/members/${this.id}`;
        await axios.get(URL, {
          headers: {
            'Content-Type': 'application/json',
            Authorization : 'Bearer ' + localStorage.token,
      ...
      async getCommittee() {
        ...
        this.loading = true;
        const URL= `http://13.244.64.153:7635/centers/${this.id}/committeeMembers`;
        // const token = localStorage.token;
        // console.log('Token is string: ' + isString(token))
        // console.log(token);
        await axios.get(URL, {
