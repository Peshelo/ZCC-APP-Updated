...
// import tabheriAddMember from './tabheriAddMember.vue'
  ...
  methods:{
    ...
    async handleSubmit(){
      ...
      // Your code for handling the form submission
          ...
          const URL = `http://13.244.64.153:7635/greaters/create/committeeMembers/makeCommittee?userId=${this.currentMember.id}&position=${this.currentPosition}&greaterId=${this.id}`;
      try{
  ...
  console.log("Error:",err.message)
      ...
      // Your code for handling the form submission
          ...
          const URL = `http://13.244.64.153:7635/admin/create?UserId=${memberId}&scopeId=${scope}&scope=NYIKA`;
      try{
       await axios.post(URL,
       {
        headers: 
  ...
  console.log("Error:",err.message)
   ...
   downloadCSV(csv, filename) {
    ...
    async getMembers() {
        ...
        this.loading = true;
        const URL= `http://13.244.64.153:7635/greaters/members/${this.id}`;
        await axios.get(URL, {
          headers: {
            'Content-Type': 'application/json',
            Authorization : 'Bearer ' + localStorage.token,
