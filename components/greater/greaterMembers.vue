      ...
      <table v-if="this.memberz == true" class="min-w-full">
              <thead class="bg-white">
                <tr>
                  ...
                  <th scope="col" class="px-3 py-3.5 text-left text-sm font-semibold text-gray-900">Nyika</th>
...
// import tabheriAddMember from './tabheriAddMember.vue'
  ...
  methods:{
    ...
    async handleSubmit(){
      ...
      // Your code for handling the form submission
          ...
          const URL = `http://localhost:8080/greaters/create/committeeMembers/makeCommittee?userId=${this.currentMember.id}&position=${this.currentPosition}&greaterId=${this.id}`;
      try{
  ...
  console.log("Error:",err.message)
      ...
      // Your code for handling the form submission
          ...
          const URL = `http://localhost:8080/admin/create?UserId=${memberId}&scopeId=${scope}&scope=NYIKA`;
      try{
       await axios.post(URL,
       {
        headers: 
  ...
  console.log("Error:",err.message)
    ...
    asPDF(){
        this.exportTableToCSV(null, 'Greater Members.csv');
      },
   ...
   downloadCSV(csv, filename) {
    ...
    async getMembers() {
        ...
        this.loading = true;
        const URL= `http://localhost:8080/greaters/members/${this.id}`;
        await axios.get(URL, {
          headers: {
            'Content-Type': 'application/json',
            Authorization : 'Bearer ' + localStorage.token,
      ...
      async getCommittee() {
        ...
        this.loading = true;
        const URL= `http://localhost:8080/taberis/${this.id}/members`;
        // const token = localStorage.token;
        // console.log('Token is string: ' + isString(token))
        // console.log(token);
        await axios.get(URL, {
