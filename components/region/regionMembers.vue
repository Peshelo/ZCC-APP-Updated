<template>
    <div>
      <!-- <div class="modal backdrop-brightness-50 overflow-y-auto flex flex-col justify-region items-region h-full" v-if="this.addMember">
      ...
      <div class="modal backdrop-brightness-50 flex flex-col items-region h-full" v-if="this.modalUser">
        ...
        <div class="bg-white text-black shadow-lg text-sm p-10 rounded-md flex flex-col items-region w-fit h-fit">
          <label class="text-region font-bold text-lg">Make Nyika Admin</label>
          <br>
                      <p class="text-center">
                        Name: {{ this.currentMember.firstname }} {{ this.currentMember.lastname }}<br>
                      </p>
                      <br>
                      <p class="text-region font-bold text-green-500">
                       Select Region
                      </p>
                      <p v-if="this.errors.currentRegion" class="text-region text-red-500">
      ...
      <div class="modal backdrop-brightness-50 flex flex-col items-region h-full" v-if="this.modal">
        ...
        <div class="bg-white border-2 text-black text-sm p-10 rounded-md flex flex-col items-region shadow-md w-fit h-fit">
          <label class="text-region font-bold text-lg">Make Committe Member</label>
          <br>
                      <p class="text-center">
                        Name: {{ this.currentMember.firstname }} {{ this.currentMember.lastname }}<br>
                      </p>
                      <form @submit.prevent="handleSubmit()" class="committee flex flex-col w-full items-region justify-region my-2">
      ...
      <table v-if="this.memberz == true" class="min-w-full">
              ...
              <tbody class="bg-white">
                <tr class="border-t border-gray-200">
                  <th v-if="tabheriMembers == null || ''" colspan="8" scope="colgroup" class="bg-gray-50 px-4 py-2 text-region h-32 text-sm font-semibold text-gray-900 sm:px-6">No items found!</th>
  ...
  console.log("Error:",err.message)
       ...
       async makeAdmin(memberId){
           ...
           if(!this.currentRegion){
              this.errors.currentRegion ="Greater is required"
           }
           if (Object.keys(this.errors).length === 0) {
            this.modal = false;
            console.log("Sending...")
      // Your code for handling the form submission
          ...
          const URL = `http://localhost:8080/admin/create?UserId=${memberId}&scopeId=${this.id}&scope=REGION`;
      try{
       await axios.post(URL,
       {
        headers: 
