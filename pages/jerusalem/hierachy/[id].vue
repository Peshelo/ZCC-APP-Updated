<template>
    ...
    <div class="px-4 sm:px-6 lg:px-8">
      <div class="sm:flex sm:items-center sm:justify-between">
        ...
        <form @submit.prevent="addNew()" class="mt-4 sm:mt-0 sm:ml-16 sm:flex-none">
          <input v-model="region" type="text" name="Region" id="region" placeholder="New Region Name" class="rounded-md border border-gray-400">
              ...
              <table class="min-w-full border-separate" style="border-spacing: 0">
                ...
                <tbody class="bg-white">
                  <tr v-for="(region,index) in regions.regions" :key="index">
                    ...
                    <td class="relative flex flex-row gap-2 text-center whitespace-nowrap border-b border-gray-200 py-4 pr-4 pl-3  text-sm font-medium sm:pr-6 lg:pr-8">
                    <NuxtLink :to="`./center/${region.id}`" class="bg-blue-500 text-white p-2 rounded-md">View Centers</NuxtLink>
    ...
    methods:{
      async getRegion() {
          ...
          this.loading = true;
         const URL= `http://localhost:8080/jerusalem/get{id}?id=${this.$router.params.id}`;
    ...
    this.loading = true;
    try{
     await axios.post('http://localhost:8080/regions/create',{
     name:this.region,
     },{
...
console.log("Error:",err.message)
     ...
     async deleteRegion(id){
      ...
      this.loading = true;
      try{
       await axios.delete(`http://localhost:8080/regions/delete?id=${id}`,{
       },{
           headers: {'Content-Type': 'application/json',
           Authorization : 'Bearer ' + localStorage.token,
           'Access-Control-Allow-Origin': '*'
