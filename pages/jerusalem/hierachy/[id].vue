<template>
  ...
  <div class="px-4 sm:px-6 lg:px-8">
    <div class="sm:flex sm:items-center sm:justify-between">
      <div class="sm:flex-auto">
        <h1 class="text-xl font-semibold text-gray-900">Showing A List of Centers</h1>
        <p class="mt-2 text-sm text-gray-700">A list of all the centers.</p>
      </div>
      <form @submit.prevent="addNew()" class="sm:mt-0 sm:ml-16 sm:flex-none flex flex-row items-center gap-2">
        <div class="sm:col-span-3">
        </div>
        <input v-model="center" type="text" name="Region" class="h-fit border border-gray-400 p-2 rounded-md" id="region" placeholder="New center name...">
  ...
  <div v-for="(center,index) in centers" :key="index" class="bg-cyan-600 p-5 w-full shadow-xl rounded-md hover:-mt-2 duration-300">
      <p class="text-white text-lg bbold mb-3">{{ center.name }}</p>
      <p class="text-xs text-gray-200">No. of greater ({{ center.greaters.length }})</p>
  </div>
  <div class="border-cyan-600 border-2 border-dashed flex flex-row justify-center items-center p-5 w-full shadow-xl rounded-md duration-300">
      <p class="text-4xl bbold text-cyan-600">+</p>
  </div>
</div>
<p class="text-md mmedium my-5">Centers Table View</p>
          <div class="shadow-sm rounded-md bg-white py-2 ring-1 ring-black ring-opacity-5">
              ...
              <tbody class="bg-white">
                <tr v-for="(center,index) in centers" :key="index">
                  ...
                  <td class="relative whitespace-nowrap border-b border-gray-200 py-4 pr-4 pl-3 text-right text-sm font-medium sm:pr-6 lg:pr-8">
                    <button @click="showModal = true" class="mx-2 text-indigo-600 hover:text-indigo-900">Committee<span class="sr-only"></span></button>
                    <NuxtLink :to="'./'+center.id" class="text-indigo-600 hover:text-indigo-900">View</NuxtLink>
                  </td>
                </tr>
              </tbody>
            </table>
