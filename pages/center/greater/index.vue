  ...
  <div class="px-4 sm:px-6 lg:px-8">
    <div class="sm:flex sm:items-center sm:justify-between">
      <div class="sm:flex-auto">
        <h1 class="text-xl font-semibold text-gray-900">Greater Table</h1>
        <p class="mt-2 text-sm text-gray-700">A list of all the greaters under {{ this.greaters.name }}</p>
      </div>
...
<p class="text-md mmedium my-5">{{ greaters.name }} Greater View</p>
            ...
            <table class="min-w-full border-separate" style="border-spacing: 0">
              <thead class="bg-gray-50">
                <tr>
                  ...
                  <th scope="col" class="sticky top-0 z-10 border-b border-gray-300 bg-gray-50 bg-opacity-75 py-3.5 pl-4 pr-3 text-left text-sm font-semibold text-gray-900 backdrop-blur backdrop-filter sm:pl-6 lg:pl-8">Greater</th>
              ...
              <tbody class="bg-white">
                <tr v-for="(greater,index) in greaters" :key="index">
                  ...
                  <td class="relative whitespace-nowrap border-b border-gray-200 py-4 pr-4 pl-3 text-right text-sm font-medium sm:pr-6 lg:pr-8">
                    <NuxtLink :to="'./'+greater.id" class="text-indigo-600 hover:text-indigo-900">View<span class="sr-only"></span></NuxtLink>
                  </td>
                </tr>
  ...
  methods:{
    async getGreater() {
       ...
       const URL= `http://3.10.190.157:7635/centers/get${scope}`;
  ...
  const scope = localStorage.getItem('scopeId');
  try{
   await axios.post(`http://3.10.190.157:7635/greaters/create?centerId=${scope}`,
   {
   name:this.greater,
   },{
