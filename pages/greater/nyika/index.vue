...
async deleteNyika(id){
      ...
      this.loading = true;
      try{
       await axios.delete(`http://3.10.190.157:7635/nyikas/delete?id=${this.id}`,{
       },{
  ...
  console.log("Error:",err.message)
      ...
      async getNyika() {
          ...
          this.loading = true;
         const URL= `http://3.10.190.157:7635/greaters/get{id}?id=${localStorage.scopeId}`;
  ...
  async addNew(){
    ...
    try{
     await axios.post(`http://3.10.190.157:7635/nyikas/create?greaterId=${scope}`,
     {
     name:this.nyika,
